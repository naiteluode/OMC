...
saltids = [row['saltid'] for row in Host.objects.values('saltid')]
...
for saltid,load in loadret.items():
    if saltid in saltids:
	    Host.objects.filter(saltid=saltid).update(load=load)
    #else:
	#    Monitor.objects.create(saltid=saltid,load=load)
for saltid,connum in connumret.items():
    if saltid in saltids:
        Host.objects.filter(saltid=saltid).update(connum=connum)
    #else:
